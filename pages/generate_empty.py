...
# Split into weeks (Mon–Fri chunks, no spillover before month start)
weeks = []
week = []
...
for w_idx, week in enumerate(weeks, start=1):
    ...
    # Header row: show weekday and date number
    header_cols = st.columns([2, 2, 2, 2, 2])
    for i, wd in enumerate(weekdays):
        if i < len(week) and week[i].strftime("%A").upper() == wd:
            date_label = week[i].strftime(f"%a %d")
            header_cols[i].markdown(f"**{date_label}**")
        else:
